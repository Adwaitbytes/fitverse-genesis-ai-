...
const FitverseNavigation: React.FC = () => {
  const navItems: NavigationItem[] = [
    { name: "Dashboard", icon: <Activity />, isActive: true },
    { name: "Workouts", icon: <Dumbbell />, isActive: false },
    { name: "Health", icon: <Heart />, isActive: false },
    { name: "Social", icon: <Users />, isActive: false },
    { name: "AI Coach", icon: <MessageCircle />, isActive: false }
  ];
  ...
  return (
    ...
    >
      <div 
        className={cn(
          "relative",
          item.isActive && "after:content-[''] after:absolute after:-bottom-1 after:left-1/2 after:transform after:-translate-x-1/2 after:w-1 after:h-1 after:bg-fitverse-blue after:rounded-full after:shadow-neon-blue"
        )}
      >
        {React.cloneElement(item.icon as React.ReactElement, { 
          className: cn("w-5 h-5", item.isActive ? "text-fitverse-blue" : "text-gray-400")
        })}
      </div>
